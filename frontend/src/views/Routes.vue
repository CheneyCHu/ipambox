...
function flash(t: string) { msg.value = t; setTimeout(() => { if (msg.value === t) msg.value = '' }, 4000) }

async function doAdd() {
  addBusy.value = true
  try {
    await api.addRoute({ destination: dest.value.trim(), gateway: gateway.value.trim() })
    flash(`✅ 已添加路由 ${dest.value}`)
...
async function doDelete() {
  ...
  try {
    ...
    flash(`✅ 已删除路由 ${deleting.value.destination}`)
    deleting.value = null
    await load()
  } catch (e: any) {
...
<template>
  <div class="animate-fade-in">
    <header class="mb-5 flex flex-wrap items-end justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">路由设置</h1>
        <p class="text-sm text-slate-400 mt-1">本机路由表查看与静态路由管理</p>
      </div>
    ...
    <div v-if="error" class="bg-red-50 border border-red-200 text-conflict rounded-xl px-4 py-2.5 mb-4 text-sm">读取路由表失败：{{ error }}</div>
    ...
    <section class="bg-white rounded-2xl shadow-card border border-slate-100 p-5 mb-4">
      <h2 class="font-semibold text-slate-800 mb-3">添加静态路由</h2>
      <div class="flex flex-wrap items-end gap-3">
        <label class="block text-sm text-slate-600 flex-1 min-w-48">目的网段
          <input v-model="dest" placeholder="如 10.0.0.0/24 或 default"
                 class="border border-slate-200 rounded-xl w-full px-3 py-2 mt-1.5 font-mono font-normal" /></label>
        <label class="block text-sm text-slate-600 flex-1 min-w-48">网关（下一跳）
          <input v-model="gateway" placeholder="如 192.168.1.1"
                 class="border border-slate-200 rounded-xl w-full px-3 py-2 mt-1.5 font-mono font-normal" /></label>
        ...
        <div v-else class="flex items-center gap-2">
          ...
          <button @click="confirmingAdd = false" class="border border-slate-200 rounded-xl px-4 py-2 text-sm text-slate-500 hover:bg-slate-50 transition">取消</button>
        </div>
      </div>
      <p class="text-xs text-slate-400 mt-2">⚠ 需要以 root 权限运行 IPAMBox；错误的路由可能导致网络中断，请确认无误后添加。</p>
    </section>
    ...
    <section class="bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden">
      <div class="px-5 py-3.5 border-b border-slate-100 flex flex-wrap items-center gap-3">
        ...
        <input v-model="search" placeholder="搜索目的地 / 网关 / 接口…"
               class="border border-slate-200 rounded-xl px-3 py-1.5 text-sm flex-1 max-w-xs" />
        <span class="ml-auto text-xs text-slate-400 tabular-nums">{{ filtered.length }} 条</span>
      </div>

      <div v-if="loading" class="py-16 text-center text-slate-400 text-sm">读取中…</div>
      <div v-else-if="!filtered.length" class="py-16 text-center text-slate-400 text-sm">没有匹配的路由</div>
      <div v-else class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr class="text-left text-xs text-slate-400 border-b border-slate-100">
              <th class="px-5 py-3 font-medium">目的地</th>
              <th class="px-4 py-3 font-medium">网关</th>
              <th class="px-4 py-3 font-medium">接口</th>
          ...
          <tbody class="divide-y divide-slate-50">
            <tr v-for="(r, i) in filtered" :key="i" class="hover:bg-slate-50/60 transition-colors">
              <td class="px-5 py-2.5 font-mono text-slate-800">
                <span v-if="r.destination === 'default'" class="bg-brand-50 text-brand-600 text-xs rounded-full px-2 py-0.5 font-medium">默认路由</span>
                <template v-else>{{ r.destination }}</template>
              </td>
              <td class="px-4 py-2.5 font-mono text-xs text-slate-500">{{ r.gateway || '直连' }}</td>
              <td class="px-4 py-2.5 font-mono text-xs text-slate-500">{{ r.iface }}</td>
              <td class="px-4 py-2.5 text-right">
                <button @click="deleting = r" class="text-conflict hover:opacity-80 text-xs font-medium">删除</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <Teleport to="body">
      <div v-if="deleting" class="fixed inset-0 z-50 bg-ink-950/40 backdrop-blur-sm flex items-center justify-center p-4"
        ...
        <div class="bg-white rounded-2xl shadow-pop w-full max-w-md p-6 animate-fade-in">
          <h3 class="font-semibold text-conflict mb-2">删除路由 <span class="font-mono">{{ deleting.destination }}</span>？</h3>
          <p class="text-sm text-slate-500 mb-1">网关：<span class="font-mono">{{ deleting.gateway || '直连' }}</span>　接口：<span class="font-mono">{{ deleting.iface }}</span></p>
          <p class="text-sm text-slate-500 mb-5">删除系统路由可能影响网络连通性（删除默认路由会直接断网），请确认后继续。</p>
          <div class="flex gap-3">
            ...
            <button @click="deleting = null" class="flex-1 border border-slate-200 rounded-xl px-4 py-2 text-sm text-slate-500 hover:bg-slate-50 transition">取消</button>
          </div>
        </div>
      </div>
    </Teleport>
