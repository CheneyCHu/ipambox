...
async function checkUpdate() {
  ...
  try {
    ...
    updateInfo.value = r
    if (!r.has_update) updateMsg.value = t('✅ 当前已是最新版本')
  } catch (e: any) { updateMsg.value = t('❌ 检查失败：') + e.message } finally { checkingUpdate.value = false }
}
...
async function applyUpdate() {
  ...
  try {
    ...
    updateMsg.value = t('✅ 已升级到 {v}，服务正在重启，约 5 秒后请重新登录…', { v: r.version })
    setTimeout(() => { token.clear(); location.href = '/login' }, 6000)
  } catch (e: any) { updateMsg.value = t('❌ 升级失败：') + e.message } finally { applying.value = false }
}
...
function flashDict(m: string) { dictMsg.value = m; setTimeout(() => { if (dictMsg.value === m) dictMsg.value = '' }, 3000) }
...
async function addItem(kind: 'dev_types' | 'owners', val: string) {
  ...
  if (list.value.includes(v)) { flashDict(t('⚠ 已存在同名项')); return }
  list.value.push(v)
  if (kind === 'dev_types') newType.value = ''; else newOwner.value = ''
  await persistDict(kind, t('已添加'))
}

async function removeItem(kind: 'dev_types' | 'owners', i: number) {
  const list = listOf(kind)
  list.value.splice(i, 1)
  await persistDict(kind, t('已删除（台账中已有记录保留原值）'))
}
...
async function persistDict(kind: 'dev_types' | 'owners', action: string) {
  try {
    ...
    flashDict(t('✅ {action}，即时生效', { action }))
...
async function commitRename() {
  ...
  try {
    ...
    flashDict(t('✅ 已重命名「{from}」→「{to}」', { from, to }) + (updated > 0 ? t('，同步更新 {n} 条台账记录', { n: updated }) : ''))
  } catch (e: any) { flashDict('❌ ' + e.message) }
}
...
async function testNotify() {
  ...
  try {
    ...
    notifyMsg.value = t('✅ 测试消息已发送，请到对应群/接口确认')
...
async function saveViewer(disable = false) {
  ...
  try {
    ...
    viewerPwd.value = ''
    viewerMsg.value = disable ? t('✅ 只读账号已停用') : t('✅ 只读账号已生效，用该密码登录即为只读模式')
...
async function doExport() {
  ...
  try {
    ...
    backupMsg.value = t('✅ 已导出 {name}', { name })
  } catch (e: any) { backupMsg.value = t('❌ 导出失败：') + e.message } finally { exporting.value = false }
}
...
async function doImport() {
  ...
  try {
    ...
    backupMsg.value = t('✅ 恢复成功，3 秒后刷新页面…')
    setTimeout(() => location.reload(), 3000)
  } catch (e: any) {
    backupMsg.value = t('❌ 恢复失败：') + e.message
...
async function doReset() {
  ...
  try {
    ...
    resetMsg.value = t('❌ 清除失败：') + e.message
      ...
      <section id="sec-notify" class="bg-white rounded-2xl shadow-card border border-slate-100 p-6 mb-5 scroll-mt-6">
        ...
        <div :class="[!notifyEnabled && 'opacity-40 pointer-events-none']">
          ...
          <div class="flex items-center gap-3 text-sm text-slate-600 mb-3">
            <span class="text-slate-400 text-xs">{{ t('推送事件：') }}</span>
            <label class="flex items-center gap-1.5 cursor-pointer">
              <input type="checkbox" :checked="notifyEvents.includes('conflict')" @change="toggleEvent('conflict')" class="accent-brand-600" /> {{ t('地址冲突') }}
            </label>
            <label class="flex items-center gap-1.5 cursor-pointer">
