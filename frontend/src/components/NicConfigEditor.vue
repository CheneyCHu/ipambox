...
async function apply() {
  ...
  if (mode.value === 'static') {
    if (!ip.value) { error.value = t('请填写 IP 地址'); confirming.value = false; return }
    cfg.ip = ip.value
  ...
  try {
    ...
    ok.value = t('✅ 已应用。若本机 IP 变化导致页面失联，请用新地址重新访问。')
...
<template>
  <div class="border border-slate-200 rounded-xl p-3 bg-slate-50/50">
    <div class="flex items-center gap-4 text-sm mb-2">
      <span class="text-slate-500">{{ family === 'ipv4' ? 'IPv4' : 'IPv6' }} {{ t('获取方式') }}</span>
      <label class="inline-flex items-center gap-1.5 cursor-pointer">
        <input type="radio" value="dhcp" v-model="mode" class="accent-brand-600" /> {{ t('自动获取（DHCP）') }}
      </label>
      <label class="inline-flex items-center gap-1.5 cursor-pointer">
        <input type="radio" value="static" v-model="mode" class="accent-brand-600" /> {{ t('手动指定') }}
      </label>
    </div>

    <div v-if="mode === 'static'" class="flex flex-wrap gap-2 mb-2">
      <input v-model="ip" :placeholder="family === 'ipv4' ? t('IP 地址，如 192.168.1.50') : t('IPv6 地址')"
      ...
      <input v-model.number="prefix" type="number" min="1" :max="family === 'ipv4' ? 32 : 128"
             class="border border-slate-200 rounded-lg px-3 py-1.5 font-mono text-sm w-24 bg-white" :placeholder="t('前缀')" />
      <input v-if="family === 'ipv4'" v-model="gateway" :placeholder="t('网关（可选）')"
             class="border border-slate-200 rounded-lg px-3 py-1.5 font-mono text-sm flex-1 min-w-36 bg-white" />
    </div>
    <p v-else class="text-xs text-slate-400 mb-2">{{ t('切换为 DHCP 后，静态地址将被清除，由 DHCP 服务器分配新地址。') }}</p>
    ...
    <div v-else class="bg-amber-50 border border-amber-200 rounded-lg p-2.5">
      <p class="text-xs text-amber-700 mb-2">{{ t('⚠ 修改网卡地址可能导致连接中断，且需要 root 权限运行 IPAMBox。确认应用？') }}</p>
      <div class="flex gap-2">
        ...
        <button @click="confirming = false" class="border border-slate-200 rounded-lg px-3 py-1.5 text-sm text-slate-500">{{ t('取消') }}</button>
      </div>
    </div>
